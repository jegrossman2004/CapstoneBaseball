...
write_csv(player_3d_metrics, "player_3d_adjustability_metrics.csv")
write_csv(contact_by_combined, "contact_by_3d_adjustability.csv")
write_csv(contact_by_combined_pitch, "contact_by_3d_adjustability_pitch.csv")
write_csv(player_by_pitch, "player_adjustability_by_pitch_type.csv")
write_csv(model_comparison, "model_comparison_3d.csv")
write_csv(pred_power, "predictive_power_3d_adjustability.csv")

# Save top/bottom adjusters
write_csv(top_adjusters, "top_20_adjusters.csv")
write_csv(bottom_adjusters, "bottom_20_adjusters.csv")

# Save the dynamically calculated pitch weights
write_csv(pitch_weights, "pitch_type_weights_calculated.csv")

cat("\n\n=== ANALYSIS COMPLETE ===\n")
cat("\nFiles saved:\n")
cat("Plots:\n")
cat("  - 3d_plot1_contact_by_adjustability.png\n")
cat("  - 3d_plot2_horiz_vs_vert_space.png\n")
cat("  - 3d_plot3_contact_heatmap.png\n")
cat("  - 3d_plot4_contact_by_adjustability_pitch.png\n")
cat("  - 3d_plot5_combined_distribution.png\n")
cat("  - 3d_plot6_player_adjustability.png\n")
cat("  - 3d_plot7_player_profiles.png\n")
cat("  - 3d_plot8_pitch_weights.png (NEW - shows calculated weights)\n")
cat("\nData files:\n")
cat("  - player_3d_adjustability_metrics.csv\n")
cat("  - contact_by_3d_adjustability.csv\n")
cat("  - contact_by_3d_adjustability_pitch.csv\n")
cat("  - player_adjustability_by_pitch_type.csv\n")
cat("  - model_comparison_3d.csv\n")
cat("  - predictive_power_3d_adjustability.csv\n")
cat("  - top_20_adjusters.csv\n")
cat("  - bottom_20_adjusters.csv\n")
cat("  - pitch_type_weights_calculated.csv (NEW - shows data-derived weights)\n")

cat("\n=== KEY FINDINGS ===\n")
cat("1. 3D adjustability metric combines horizontal + vertical with pitch-specific weights\n")
cat("2. Player profiles show who can adjust in full 3D vs specialists\n")
cat("3. Model comparison reveals if combined metric is sufficient vs separate planes\n")
cat("4. Predictive power analysis shows contact rate gradient across adjustability spectrum\n")
cat("5. Elite adjusters identified - can be used for scouting/development\n\n")

cat("=== PITCH-TYPE WEIGHTS (CALCULATED FROM YOUR DATA) ===\n\n")
cat("These weights were derived by fitting logistic regression models to each pitch type.\n")
cat("They automatically adjust based on your data and will change if you use different datasets.\n\n")

for (i in 1:nrow(pitch_weights)) {
  ptype <- pitch_weights$pitch_group[i]
  hw <- pitch_weights$horiz_weight[i]
  vw <- pitch_weights$vert_weight[i]
  
  cat(sprintf("%-15s: Horizontal = %.1f%%, Vertical = %.1f%%\n", 
              ptype, hw * 100, vw * 100))
  
  if (hw > 0.52) {
    cat("                → Horizontal adjustability is more critical for this pitch type\n")
  } else if (vw > 0.52) {
    cat("                → Vertical adjustability is more critical for this pitch type\n")
  } else {
    cat("                → Both planes are equally important for this pitch type\n")
  }
}

cat("\nThese weights determine how horizontal and vertical angle differences\n")
cat("are combined into the unified 3D adjustability score.\n\n")